 ...
 */
contract EduBatcher is AccessControl {
    address constant MOCK_ETH_ADDRESS =
        0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;

    /*
     * @notice Constructs the EduBatcher contract and grants the RESCUE_ROLE to the contract deployer.
    ...
    struct ExecuteRequest {
        IEdu.ExecutionDetails executionDetails;
        IEdu.MessageDetails messageDetails;
    }
     ...
     * @param EduAddress_ address of Edu
    ...
    function sealBatch(
        ...
        _sealBatch(EduAddress_, sealRequests_);
    }

    /**
     * @notice propose a batch of packets sequentially by EduDestination
     * @param EduAddress_ address of Edu
     * @param proposeRequests_ the list of requests with packets to be proposed by EduDestination
     */
    function _proposeBatch(
        ...
        for (uint256 index = 0; index < proposeRequestLength; ) {
            IEdu(EduAddress_).proposeForSwitchboard(
    ...
    function proposeBatch(
        ...
        _proposeBatch(EduAddress_, proposeRequests_);
    }
    ...
    function sendBatch(
        ...
        _proposeBatch(EduAddress_, proposeRequests_);
        _attestBatch(attestRequests_);
        _executeBatch(EduAddress_, executeRequests_);
    }
    ...
    function _executeBatch(
        ...
        for (uint256 index = 0; index < executeRequestLength; ) {
            ...
            IEdu(EduAddress_).execute{value: msgValue}(
    ...
    function getProposalCountBatch(
        ...
        for (uint256 index = 0; index < packetIdsLength; ) {
            uint256 proposalCount = IEdu(EduAddress_).proposalCount(
                packetIds_[index]
            );
